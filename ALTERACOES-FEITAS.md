...
-- Bloqueio real no banco: o mesmo barbeiro não pode ter dois horários ativos iguais.
...
begin
  ...
  values (
    new.id,
    coalesce(new.raw_user_meta_data->>'gmail', new.raw_user_meta_data->>'public_email', new.email, ''),
...
-- Usado pela tela de agendamento para desabilitar horários já ocupados sem expor dados de outros clientes.
create or replace function public.get_booked_slots(p_barber_id uuid, p_appointment_date date)
